...
fileInput.addEventListener("change", async (e) => {
  ...
  showStatus("Importing document and building your workspace...", "");
  ...
  try {
    ...
    showStatus(`✓ "${file.name}" is ready for queries.`, "success");
  } catch (err) {
    showStatus(`✗ ${err.message}`, "error");
  } finally {
    uploadBtn.disabled = false;
    uploadBtn.innerHTML = `
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12"></path><path d="M8 11l4-4 4 4"></path><path d="M4 19h16"></path></svg>
      Load File
    `;
...
function renderFileList() {
  ...
  if (notebooks.length === 0) {
    fileListEl.innerHTML = `<div class="empty-state">Library empty — add a document to begin.</div>`;
    return;
  }
...
function switchNotebook(id) {
  ...
  activeFileName.textContent = `File: ${nb.name}`;

  chatInput.disabled = false;
  sendBtn.disabled = false;
  chatInput.placeholder = `Ask about "${nb.name}"...`;
  ...
  if (nb.messages.length === 0) {
    chatMessages.innerHTML = `
      <div class="welcome-screen">
        ...
        <p>Your file <strong>"${nb.name}"</strong> is indexed and searchable. Start with a question or ask for a summary.</p>
      </div>
    `;
    return;
  }
